...
<!-- <input type="checkbox" bind:checked={todo.done} on:click={() => handleCheck(todo.id)} > -->
<input type="checkbox" bind:checked={todo.done} on:click={() => handleCheckTodoProp(todo.id)} >
{#if editMode === todo.id}
